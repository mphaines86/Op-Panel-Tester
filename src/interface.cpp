...
static uint8_t debounce(uint8_t portRegister, uint8_t port, uint8_t row, uint8_t column){
  //cool++;
  uint8_t output = 0;
...
void interfaceCheck(){
  ...
  if(interface.sourceNumber>-1){
    switch(interface.activeMenu){
      case ptMenu:
        if (interface.sourceNumber>=10)
          handleMenuInput(&interfaceParameters[interface.activePage][interface.sourceNumber-9]);
        break;
      case ptParam:
        handleParamInput();
        break;
